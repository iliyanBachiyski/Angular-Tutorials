...
application.service('WeatherService',['$http', '$log', function ($http, $log) {
    ...
    var FIVE_DAYS_FORECAST_URL = URL + 'forecast' + CITY_PARAM_EXTENSION + OPENWEATHER_API_KEY;
    ...
    self.getFiveDaysWeatherByCity = function (city) {
        return $http.get(FIVE_DAYS_FORECAST_URL.replace('{city_param}',city))
            .then(function (result) {
                if(result.status === 200) {
